...
FULL_DIR = "/home/f25-echo/echo_fsw/data/camera/photos/full"
...
def handle_cap_and_downlink(rfm9x, rap_data, pid, sid, flag):
    ...
    # Always use latest thumbnail instead of requested filename
    request_full = ("FULL" in arg_str.upper())

    if request_full:
        # Choose latest full-res image
        try:
            files = sorted(
                [f for f in os.listdir(FULL_DIR) if f.lower().endswith(".jpg")],
                reverse=True,
            )
            latest = os.path.join(FULL_DIR, files[0]) if files else None
        except:
            latest = None

        if latest is None:
            print("[RADIO] No full images available, falling back to thumbnail.")
            request_full = False  # fallback to thumbnail
    ...
    print(
        f"[RADIO] GS requested '{requested_fname}' "
        f"({ 'FULL' if request_full else 'THUMBNAIL' }) "
        f"→ sending: {latest}"
    )
