...
# Updated paths (f25-echo2)
BASE_DIR  = "/home/f25-echo2/echo_fsw/data/camera/photos"
FULL_DIR  = f"{BASE_DIR}/full"
THUMB_DIR = f"{BASE_DIR}/thumb"
VIDEO_DIR = "/home/f25-echo2/echo_fsw/data/camera/videos"

# ---- Downlink image config (the "thumb" is what gets downlinked) --------
# Bumped from 640x480 @ Q75 to 1024x768 @ Q85 for better ground visibility.
# Typical resulting file size: ~150-250 KB.
DOWNLINK_WIDTH   = 1024
DOWNLINK_HEIGHT  = 768
DOWNLINK_QUALITY = 85
...
class CameraWorker:
    ...
    Takes a full-res picture every N seconds and saves:
      - full:  full-resolution image (archival)
      - thumb: 1024x768 JPEG at quality 85 (for downlink)
    ...
    def __init__(self, period_seconds=10, altitude_callback=None):
        ...
        self._thread = None
        self._altitude_thread = None
        self.altitude_callback = altitude_callback

        # Camera lock to prevent photo/video conflicts
        self._camera_lock = threading.Lock()

        # Track which altitude videos have been captured (feet)
        ...
        # Last altitude seen, for edge-triggered video logic
        self._last_altitude = 0
        self._altitude_check_interval = 1.0
    ...
    def start(self):
        ...
        if self.altitude_callback:
            self._altitude_thread = threading.Thread(
                target=self._altitude_loop, daemon=True
            )
    ...
    def _loop(self):
        ...
        while not self._stop.is_set():
            ...
            # Use the stop event as sleep so shutdown is responsive
    ...
    def _altitude_loop(self):
        ...
        while not self._stop.is_set():
            try:
                if self.altitude_callback:
                    current_alt = self.altitude_callback() or 0

                    # Snapshot the dict to avoid mutating-while-iterating
                    # when _trigger_altitude_video() sets a key.
                    for target_alt, captured in list(self._videos_captured.items()):
                        if (not captured
                                and current_alt >= target_alt
                                and self._last_altitude < target_alt):
            ...
            self._stop.wait(self._altitude_check_interval)
    ...
    def _capture(self):
        """Capture full-res photo and generate downlink-sized thumbnail."""
        ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")

        full_path       = f"{FULL_DIR}/img_{ts}.jpg"
        thumb_temp_path = f"{THUMB_DIR}/img_{ts}_thumb_temp.jpg"
        ...
        # ---- thumbnail: downscale the full image with PIL ----
        # We use the already-captured full image instead of invoking rpicam-still
        # twice. This saves ~1s per cycle, avoids a second sensor warmup, and
        # guarantees the thumb corresponds to the same moment as the full image.
        try:
            img = Image.open(full_path)
            img.thumbnail(
                (DOWNLINK_WIDTH, DOWNLINK_HEIGHT),
                Image.Resampling.LANCZOS,
            )
            img.save(
                thumb_path,
                "JPEG",
                quality=DOWNLINK_QUALITY,
                optimize=True,
                progressive=True,
            )
            ...
            log_event("CAMERA_CAPTURE_OK", {
                "full": full_path,
                "thumb": thumb_path,
                "full_size_kb":  round(full_size  / 1024, 1),
                "thumb_size_kb": round(thumb_size / 1024, 1),
                "thumb_dims": f"{DOWNLINK_WIDTH}x{DOWNLINK_HEIGHT}",
                "thumb_quality": DOWNLINK_QUALITY,
            })

        except Exception as e:
            log_event("CAMERA_OPTIMIZE_FAIL", {"error": str(e)})
            # Best-effort cleanup of temp file from older code paths
        ...
        # Report storage state (never deletes, just warns if over soft cap)
        warnings = cleanup_oldest_if_needed()
        if warnings:
            log_event("CAMERA_STORAGE_WARN", {"warnings": warnings})
    ...
    def manual_video(self, duration_seconds=15, filename_prefix="manual"):
        """Manual video capture, e.g. for bench testing."""
